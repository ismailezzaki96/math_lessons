...
dbasePath = 'filters/dbase'  # path to abbrev dbase file
...
regex = re.compile(
    '^\+|^\(\+|^\[\+|^\{\+'
)  #create pattern to detect abbreviations, starting with a + but may be preceded by some sort of bracket/parenthesis etc
...
def abbreplace(key, value, format, meta):
    ...
    for k, v in meta.items():  #get keys, values for all metadata
        if k.startswith('+'):  # does the key start with a plus sign?
            abbrevlist[str(k.strip('+'))] = stringify(
                v['c'])  # add key and value to the abbrev dictionary

    if key == 'Str' and re.match(
            regex, value
    ):  # is the string an abbrev? (starts with a plus/bracket? see line 20)
        bare = value.strip(
            '.,;:(()[]{}'
        )  # get the bare abbrev string by stripping potential punctutation
        rp = value.lstrip('.,;: ()[]{}')  # strip punct chars from left
        rp2 = rp.lstrip(
            bare
        )  # strip abbrev from left, leaving only punct chars on right of abbreviations
        lp = value.rstrip('.,;: ()[]{}')
        lp2 = lp.rstrip(bare)  # same as previous two lines but on LHS

        if bare.strip(
                '+') in abbrevlist:  # check the key(ie abbrev) is defined
            return Str(
                lp2 + abbrevlist[bare.strip('+')] + rp2
            )  # send back the 'value' of the abbrev 'key', adding stored punctuation
